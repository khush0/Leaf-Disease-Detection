...
load('Testfea.mat')
load('Trainfea.mat')
knntrain = fitcknn(Trainfea,classes,'NumNeighbors',3); % multisvm is the trained model. save it at end for doing testing
save('knntrain.mat','knntrain');
load('knntrain.mat');
Groupnb = predict(knntrain,Testfea);
if Groupnb == 0   
    %set(handles.edit1,'string','Un_Ripe')
    msgbox('Identified  type "Alternaria Alternata" by KNN classification');
elseif Groupnb == 1
  %set(handles.edit1,'string','Semi_Ripe')
  msgbox('Identified  type "Anthracnose" by KNN classification');
elseif Groupnb == 2
  %set(handles.edit1,'string','Fully_Ripe')
  msgbox('Identified  type "Bacterial Blight" by KNN classification');
elseif Groupnb == 3
  %set(handles.edit1,'string','Fully_Ripe')
  msgbox('Identified  type "Cercospora Leaf Spot" by KNN classification');
elseif Groupnb == 4
  %set(handles.edit1,'string','Fully_Ripe')
  msgbox('Identified  type "Healthy Leaf" by KNN classification');
